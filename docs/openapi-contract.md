...
OpenAPI（旧 Swagger）の使い方には大きく分けて 2 つある。
...
## 3. このプロジェクトが Spec-First を選んだ理由

| 動機 | 説明 |
|------|------|
| **学習プロジェクト**として OpenAPI 自体を学びたい | yaml を手書きする経験は教育価値が高い |
| **フロント先行**で開発したい | BE 未着手でも UI が動く、MSW モードのおかげ |
| **BE が複数言語でも再利用したい** | yaml が言語非依存。Spring Boot から Go に変えても yaml は不変 |
| **議論の中心を yaml にしたい** | コードレビューより仕様レビューの方が早い |

つまり「**yaml が書けると嬉しい・コードに縛られたくない**」というモチベーションがあるなら Spec-First。「**Java の知識しかなく yaml は副産物で十分**」なら Code-First。
