...
| L7 実行速度 | 3 | 既定は単一ブラウザの逐次実行で、`cypress run` の cold start に数秒、フル動作で spec あたり 10-30 秒。並列実行は Cypress Cloud / `--parallel` の sharding が前提（docs.cypress.io > Parallelization + 体感） | 5 | `fullyParallel: true` の worker 並列実行（playwright.dev > Parallelism）と複数ブラウザ並走が標準。コミュニティ計測では Cypress 比 2-3 倍高速の報告が多いが spec 数・並列度・対象ブラウザ数で振れるため数値は範囲幅で受け取る（公式 + 体感） | 4 | parallel runner で複数 capability 並走が可能だが、WebDriver プロトコル経由のオーバーヘッドで Playwright の CDP 直叩きには劣る。Cypress の単一ブラウザよりは速い（公式 + 体感） |
...
- **差分**:
  ...
  - L4 デバッグ体験: Cypress の time-travel snapshot は **開発中のライブ DOM 探索**（`cypress open` の対話操作）に強く、Playwright Trace Viewer は **事後解析**（CI 失敗の trace zip を後から `npx playwright show-trace` で開く）に強い。学習者は前者で「テスト書きながら DOM を覗く」、後者で「CI が落ちた理由を再現なしに読み取る」を学べる。客観点数は同点（5 / 5）だが用途分担は別軸として §2.5.3 評価観点 3 で読み合わせる。WebdriverIO は `browser.debug()` REPL に留まり 1 段見劣りする（3）
  ...
  - **ブラウザ網羅（採点表外の補足）**: Cypress は Chromium 系 + Firefox を 1st-class、WebKit は experimental サポート。Playwright は Chromium / Firefox / **WebKit を 1st-class** で備え、`devices` プリセットで iOS / Android Mobile emulation も標準。本プロジェクトは Ionic で iOS Safari（WebKit）もターゲットに含むため、CI で WebKit を回す要件が将来出れば差が決定的になる（現状の MSW + dev サーバ運用では Chromium 単一で足りるため採用判断には反映していない）。WebdriverIO は service 経由で Appium / 実機網羅が強いが、ローカル CI でのブラウザ網羅は Playwright に劣後
- **本プロジェクトでの選択理由**: 親戦略書 §1.1「**E2E は Cypress + Playwright 比較で十分。学習サンプルとしての差別化が薄い（WebdriverIO 不採用理由）**」を直接の根拠とし、(1) L3 既存親和性の差（Cypress=5 / Playwright=3 / WebdriverIO=2）が決定打、(2) Cypress の time-travel と Playwright の Trace Viewer は「同シナリオを 2 手法で比較する」教材として読み合わせ価値が高く §2.5.3 プロトタイプ設計で `items-flow` 動線を両者で揃える方針、(3) WebdriverIO は L1 / L3 / L4 すべてで Cypress・Playwright に対して劣後し、客観評価でも「要注意」帯のため学習サンプルとしての残置も見送る、の 3 点。客観評価では Cypress=4.58（優秀）/ Playwright=4.42（良好）と両者が上位帯のため、Playwright を比較サンプル枠で残す判断は妥当。**ただし両者の総合スコア差 0.16 はほぼ L3 既存親和性（5 対 3）に集約されており**、新規プロジェクトで本書を流用する場合の含意は学習者向けメモを参照
- **学習者向けメモ**: まず**新規プロジェクト視点での参考スコア**として、L3 既存親和性（重み 1.5）を除外した加重平均は **Cypress 4.50 / Playwright 4.68** で順位が逆転する（Cypress 分子 45.3 − 5×1.5 = 37.8、Playwright 分子 43.8 − 3×1.5 = 39.3、共通分母 9.9 − 1.5 = 8.4）。本プロジェクトでの Cypress 採用は「既存 `cypress.config.ts` + mochawesome / junit パイプが稼働中」という L3 の差が決定打であり、**新規プロジェクトで本書の評価を流用するなら Playwright を第一候補に置く根拠が立つ**点を学習者は意識してほしい。以下のいずれかに該当する場合は Playwright / WebdriverIO への切り替えを検討する余地がある: (a) **Playwright**: テストスイートが数百本規模に膨らみ CI 実行時間が学習継続性を下げ始めた場合（L7 並列実行のメリットが顕在化）、**iOS Safari（WebKit）や Mobile emulation を CI で回したくなった場合**（Ionic の iOS ターゲット込みで E2E を保証したい場面）、Vue/Vite と独立した「フレームワーク非依存の E2E」を学びたい場合、API テスト + Component Testing + Visual Regression を 1 ツールで統合したい場合。本書 §2.5.3 のプロトタイプサンプル 1 本で書き心地を体感できる。(b) **WebdriverIO**: モバイル（Appium）+ デスクトップ（Electron）+ Web を同一テストランナーで束ねたい場合、Sauce Labs / BrowserStack のクラウド実行を主目的にする場合、Cucumber BDD と E2E を統合したい場合。Web 単独の SPA テストで採用するメリットは薄いため、本プロジェクトの教材枠では §2.5.1 採点表で「不採用判断の客観根拠」を学ぶ材料として残す

#### 2.5.3 プロトタイプ設計（Cypress vs Playwright）

> ※ 本節は比較サンプル実装前の机上設計。`frontend/__samples__/comparison/playwright/items-flow.spec.ts` 実装（親戦略書 P3 完了後）に L1 学習コスト / L4 デバッグ体験 / L7 実行時間 / 下記評価観点 5（ブリッジ差し替え）を体感ベースで再採点する余地を残す。
...
- **評価観点**:
  ...
  3. デバッグ体験（Cypress Test Runner の time-travel = ライブ DOM 探索 vs Playwright Trace Viewer = 事後 zip 解析）の用途分担
  4. CI 実行時間（Phase 8 採用時の参考データ）
  5. **Capacitor ブリッジ差し替え方式**: 採用品（Cypress）は親戦略書 §2.4 に従い `cy.stub(window, 'Capacitor')` で `BridgeDemo` 経由のブリッジ呼び出しを差し替える。比較サンプル（Playwright）は (a) `page.addInitScript` で `window.Capacitor` をテスト起動時に注入する方式、(b) `page.exposeFunction` で個別 API を差し替える方式の 2 通りが候補で、どちらが学習者に読みやすいか・既存 `useDemoSdk.spec.ts` の `vi.mock` パターンとの読み合わせ価値が高いかを比較サンプル実装時に評価する
...
#### 学習導線軸（L1/L2/L3）と成熟度軸（L7/L8）の相関

採用品 11 候補と不採用候補 9 候補の採点を見比べると、**本プロジェクトの採否を分ける主要因は L3 既存親和性**（採用品は概ね 4-5、要注意帯に落ちた不採用候補は概ね 2-3）であり、L7 実行速度・L8 エコシステムの成熟度軸は単独では採否を決めない（例: Jest は L8=5、nock は L8=4 と高得点だが L3=2-3 で総合スコアを下げて不採用）。学習サンプル集という目的設定上、既存パイプとの接続性が他軸より重く効くのは設計どおりの結果。

#### 客観評価が「良好」以上に達した不採用候補

§3.3 不採用候補 9 つのうち、客観評価「良好」（≥3.5）に達したのは **4 候補**: Jest (3.85) / happy-dom (3.68) / nock (3.68) / axios-mock-adapter (3.53)。これらは「ツールとしては十分使えるが、本プロジェクト内で採用品との差別化が薄い」候補群。親戦略書 §6.4 Open Items #4「比較サンプル CI 実行の重さ」を判断する際、これら 4 候補を比較サンプル枠として追加採択する価値は定量的に存在する（特に Jest と happy-dom は L1 学習コストも高得点で教材化しやすい）。

#### 学習者の次の手（学習サンプル拡張の優先順位）

§3.3 不採用候補のうち最高得点は **Jest (3.85)**。次点は happy-dom / nock (3.68)。学習サンプル拡張の優先順位はこの順を推奨:

1. **Jest**: Vue 3 + Vite 文脈で「なぜ Vitest を選んだか」を体験的に理解できる。トランスフォーマー多重化（babel-jest + vue-jest + ts-jest）の躓きを実感することで §2.1 採否解説の L3 根拠が腹落ちする
2. **happy-dom**: Vitest 設定で `environment` を切り替えるだけで試せる軽量さ。Ionic Shadow DOM 描画の安定性差を実測する教材として有効
3. **nock**: Node スクリプトで HTTP 介入を学ぶ際の参照点。SSR / バックエンド系の学習に転用可

#### 改訂提案

採点中、客観評価で「優秀」（≥4.5）に達した不採用候補は **なし**（最高は happy-dom 3.68）。したがって親戦略書 §1 決定表および §1.1 不採用候補メモを覆すような発見はなく、**改訂提案は該当なし**。本書の採点結果は親戦略書 §1 の決定と整合する。

ただし §6.4 Open Items #4「比較サンプル CI 実行の重さ」については、本書 §3.4 上記の「良好以上 4 候補」を比較サンプル枠に追加採択する余地があり、検討の材料として申し送る。
